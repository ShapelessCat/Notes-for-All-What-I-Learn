...
* TODO Chapter 8: Basic Type Classes and Their Usage - 93 - _START_
  ...
  - The _Cats_ library for purely functional programming and its structure
  - /Type classes/ _Cats_ defines
...
* TODO Chapter 9: Libraries for Pure Functional Programming - 144
** Cats effect - 145
*** ProductR - 145
...
* TODO Chapter 10: Patterns of Advanced Functional Programming - 209
  - patterns :: common solutions to common problems that repeat from project to prorject.

  - /Functional programming/ also has its own /patterns/.
    _Since it has certain problems and challenges UNIQUE to it, it will also have
    UNIQUE solutions._ =from Jian= UNIQUE here includes good and bad!
      In this chapter, we will DISCUSS _solutions_ to common functional programming
    problems.
...
** TODO Monad Transformers - 209 - =READING...=
   /Monad transformers/ is an IMPORTANT _patterns of purely functional programming_
   that allows us to *combine* _DIFFERENT (=from Jian= an important missing word
   in the book)_ /effect types/.

*** DONE The specialization of effect types - 210
    CLOSED: [2023-03-20 Mon 01:45]
    - We've discussed HOW /effect types/
      * are ubiquitous in purely functional programming and =TODO= =???=
      * are used in order to *abstract away* /side effects/.


    - =IMPORTANT=
      You may have also noticed that these /effect types/ are *highly specialized*,
      which means we have _ALMOST one-to-one mapping between /side effects/ and
      /effect types/._

    - For example,
      * ~Option~ is the /effect type/ that is capable to represent
        _POSSIBLE MISSING values_.

      * *HOWEVER*,
        it does *NOT perform well* when tasked with modeling errors and
        exceptions (=from Jian= if we want to keep the related info) --
        it does *NOT preserve* the information of the nature of the failure.
        + =from Jian=
          If we don't want to keep the error or exception info or if missing
          result is the only error can happen, use ~Option~ in this scenario is
          also okay. This is why in cats there are defined instances for
          ~ApplicativeError[Option]~ and ~MonadError[Option]~.

      * ~Either~ is GOOD for *modeling* _erroneous computation_ that can fail.
        *HOWEVER*, if you try to model a computation that may signal missing
        result with ~Either~, you will find that this data type is *REDUNDANT*
        for these purposes -- whenever you have a computation that does NOT
        return a value, _you will *STILL NEED* to return something from it._ In
        the erroneous scenario, we could have done the following:
        #+begin_src scala
          def _foo(x: Int): Option[Int] =
            if x < 0
            then None
        ...
        #+end_src
        + We can add a message into the above ~Left~ value to point out the computation
          didn't produce any result.
          - This usage of ~Either~ is rather contrived!

    - =IMPORTANT= *CONCLUSION*:
      The above example showss that /side effects/ are fine-grained, and every
      /side effect/ has its own representing /effect type/.
      * This is natural situation for Scala, as for any strongly-typed language.
        + =from Jian= ???
          Should this "strongly-typed language" be "staticly-typed language".

      * =IMPORTANT= *FOLLOWUP CONCLUSION*:
        The finer your type system matches the /side effects/, the more
        /side effects/ you can model out of the box.

        + =IMPORTANT= This *FOLLOWUP CONCLUSION* leads to a question we need answer:
          _WHEN_ we have *more than one* /side effect/,
          we need to think about _HOW_ are we going to *present* and *combine* them.

*** TODO An application with multiple side effects - 211
    Modern applications aim for asynchrony.

**** TODO Asynchrony - 211 - =TODO: NOTE=
     Illustrate this with an example:
     a web server tasked with handling incoming HTTP requests in a setting of
     a web application that manages an online forum.

     - *Requirement 0*:
       Consider that the requests in question are to be responded to with the
       data about all of the forum posts present in the database. This means that
       _IN ORDER TO reply to any given HTTP request,
       we first need to *obtain* the required data from the database_.
       ...
       * The solution may look as follows:
         ...
         The types and methods are declared as follows:
         #+begin_src scala
           type Request
           type Response
           type Post
     ...
     - *Requirement 1*:
       _HOWEVER_, the above task is *NOT the ONLY task* that our application needs
       to do.
       Consider also that our forum is private, which means
       _we need to *authenticate* a user PRIOR TO *giving* them access to the
       forum._
       * Solution popular nowadays:
         Use some cloud based authentication services -- don't implement the authentication
         mechanism oon the sever side, but outsource it to some provider.

         *This is the mechanissm known as /Authentication as a Service (AaaS)/.*
         + Benefits:
           =TODO=
           =TODO=
           =TODO=

       * To use this solution we need to answer some questions:
         =TODO=
         =TODO=
         =TODO=
...
**** DONE The side effect of errors - 214
     CLOSED: [2023-03-20 Mon 13:51]
     - *Things do not always go smoothly, and they can go wrong on multiple levels*.
       * Things can fail because unexpected reasons.
         + When we connect to the _cloud authentication service_,
           the connection itself may go wrong.

           For example,
           - the server reply _times out_.

             OR

           - your _application credentials are wrong_ and you do not end up accessing
             the feature of the cloud authentication service that you would like
             to access.

       * Business logic of the application allows for some errors.
         + Anyone can request an access, but they may not be authorized.
           When this happen, we need to reply with an error message instead of
           the data they requested:
           #+begin_src scala
             if authenticated
             then allPosts.map(respond)
             else Future(respond("You are not authorized to perform this action"))
           #+end_src

     - All of the cases discussed here are a clear evidence that we have
       *a /side effect/ of an error* here as a potential.
       _We would abstract away this /side effect/ into an /effect type/._
       * Q ::
         We already have ~Future~ for the asynchrony effect.
         How do we introduce another /side effect/ here?

       * A :: ~Future[Either[A, B]]~ -- *stack* the /effect types/.
         + Scala standard library ~Future~ itself provides a capability for error
           reporting. _HOWEVER_, this is a detail specific to the ~Future~'s
           implementation --
           it is perfectly possible to imagine /asynchronous primitives/ that
           provide you with the abstraction of asynchrony
           but do *NOT* catch the errors that happen inside them.

         + So here, we will be viewing the situation
           _AS IF_ the ~Future~ does *NOT* provide the capability of error-handling.

     - Q ::
       Can we use the ~Future[Either[A, B]]~ type and use non-nested simple
       for-comprehension to implement our requirement?
       * Have a try (failed):
         #+begin_src scala
           for {
         ...
         + Error message:
           #+begin_src text
             found   : Either[String, Boolean]
             required: Boolean
                    if (authenticated) allPosts.map(respond)
                        ^
           #+end_src

         + Rewrite this /for-comprehension/ with combinators ~flatMap~, ~map~,
           and ~filter~ can help us to understand the root cause of the above
           failure:
           The ~flatMap~ comes from ~Futrue~, and it is reasonable to not take
           care the inside ~Either~. Actually, the ~flatMap~ can't know if there
           is an inside ~Either~ or other types.
           * ~def flatMap[S](f: T => Future[S])(implicit executor: ExecutionContext): Future[S]~

         + After knowing the reason, we can fix it:
           #+begin_src scala
           ...
           #+end_src
           Here, we *MANUALLY extract* the result value from ~Either~.
           ...
           =IMPORTANT=
           =IMPORTANT=
           - *WHAT WE FOUND from this Solution*: =IMPORTANT=
             Immediately, we can say that *something is VERY WRONG* and needs to
             be corrected.

             * The feeling of something wrong arises
               _BECAUSE_ previously we have discussed that the very point of having
               /Monadic flows/ and the ~flatMap~ function is *TO abstract away*
               /effect types/. Yet here, we need to work with them EXPLICITLY.

*** TODO Monad Transformers - 218
*** TODO Generalizing the pattern - 220
...
** TODO Type-level programming - 232
   - The *compiler* translates your program
     _FROM_ one set of instructions _INTO_ another.

   - In *high-level languages*, you
     _TRANSLATE_ higher-level instructions that are easy to read and write for people
     _TO_        lower-level instructions that are easy for machines to execute.

   - Since the compiler needs to perform a conversion of one set of symbols into
     another set of symbols, it builds some internal model of the program that
     you are writing. In a sense, we can say that the compiler understands the
     program, for some definition of understands.

   - If the compiler builds an internal model or understands your program in some
     other way, we can also harness the power of compiler to make the compiler
     check for the correctness of the program.

     We can make the compiler impose and enforce certain styles or guarantees
     that your program must obey.

     We have already seen an example of annotation.tailrec.

     The job of the annotation was to instruct the compiler to check the
     annotated function to have certain guarantees about it.

     Concretely, the compiler checked the function to be tail-recursive.

   - To ensure correctness of your program, we may well use a strongly typed
     language, and encode the guarantees and the semantics of your program in
     types.

     The types are known to the compilers, and hence, it can perform certain
     checks on these types.

     For example, it can make sure that you supplied an argument of the correct
     type to a function because it knows the input type of the function.

     So, in a strongly-typed language, no longer can you make the mistake of
     passing an argument of the wrong type to a function.

   - The advantage of having strong compilers that check the program for mistakes
     is that you are able to catch more errors at compile-time. In general,
     errors that are caught at compile- time are much easier to debug and fix.
     If an error happens at runtime, it means that it is not discovered right
     away. You may release your application to the end user, and for
     particularly tricky bugs, months and even years can pass before they are
     discovered. Once they are discovered, you need to investigate the erroneous
     behavior yourself throughout the codebase and try to reproduce it so that
     you can fix it.

   - Compile-time errors are manifested right away. When you compile the code, you
     see exactly how and where you went wrong. So there is an obvious advantage
     to making sure the compiler can catch as many errors as possible.

   - Can we push our compiler even further? Let's have a look at an example of the
     type-level programming in Scala. It is called type-level because we aim to
     encode as many guarantees about our program as possible in the types. This
     way, these guarantees are checked at compile-time.

*** A naive implementation of the heterogeneous list - 233
*** Type-level solution to the heterogeneous list problem - 236
    The _STRATEGY_ for solving this problem is to *ENCODE the guarantees that we want to have in /types/.*

    - We will leverage the highly-developed mechanism of /implicit resolution/ to
      enforce that guarantee on a certain type.

    - The general idea is to have _a guarantee presented by an /implicit value/:_
      if we need a certain guarantee about our program, we make our program
      implicitly depend on the value that represents this guarantee.

    - The catch is that the application will not compile if the compiler does not
      find any of the implicits the program depends on. So, by making the
      program depend on the implicits that represent our guarantees, and making
      sure these values are present in scope only if the guarantee is satisfied,
      we can make sure is that the compiler will not compile a program that does
      not satisfy this guarantees.
