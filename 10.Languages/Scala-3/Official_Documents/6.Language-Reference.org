...
#+VERSION: 3.0.2-RC1-bin-20210626-f33bc8d-NIGHTLY
...
**** DONE Transparent vs. non-transparent inline
    ...
    - As we already discussed, /transparent inline methods/ may *influence* /type
      ...
      * /transparent inline methods/ must be *expanded DURING* /type checking/ of
        the program.
    ...
    - A noteworthy difference is the behavior of ~transparent inline given~.
      ...
      - A ~transparent inline given~ can add a type ascription in its RHS (as in
        ~f2~ from the previous example) to
...
**** DONE Inline Conditionals
     ...
     - In a ~transparent inline~,
       ...
       condition *during* /type checking/.
...
**** DONE The ~scala.compiletime~ Package
     CLOSED: [2021-04-28 Wed 03:38]
     The ~scala.compiletime~ package contains _helper definitions_ that provide
     support for /compile-time operations/ over _values_. They are described in
     the following.
...
**** DONE Reference - =READ=
     ...
     - =PR #4768=, which explains how ~summonFrom~'s _predecessor_
       (/implicit matches/) can be used for /typelevel programming/ and /code
       specialization/

     - =PR #7201=, which explains the _NEW ~summonFrom~ syntax._

*** DONE Macros
    CLOSED: [2021-02-14 Sun 02:27]
...
**** DONE Variant: Indentation Marker ~:~
     ...
     - To turn this feature on, use language import
