...
- *Lifetime of the MutexGuard*
  ...
  sometimes lead to subtle surprises.

  * If we _assign the /guard/ a name_ with a ~let~ statement (as in our examples above),
    it's relatively STRAIGHTFORWARD to see when it will be dropped.
  ...
  * Any temporaries produced within a larger expression, such as the /guard/
    ...
    While this might seem obvious and reasonable, it leads to a *COMMON PITFALL*
    that usually involves a ~match~, ~if let~, or ~while let~ statement. Here is
    an example that runs into this pitfall:
    #+begin_src rust
