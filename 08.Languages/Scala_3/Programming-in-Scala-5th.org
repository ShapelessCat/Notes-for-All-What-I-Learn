...
- In this chapter, we'll
  ...
  * Along the way we'll demonstrate some of the techniques for information
    hiding introduced in Chapter 12 by using a concrete example: the design of a
    class for purely functional queues.
...
- ONE GOAL:
  ...
  * A :: Ideally, a /functional (immutable) queue/ should _NOT_ have a
    ...
    That is, all three operations, ~head~, ~tail~, and ~enqueue~, should operate
    in *constant time*.
...
*** An alternative: private classes - 399 - =IMPORTANT= =REVIEW=
...
- ~Queue~ is a /type constructor/, or called /higher kinded type/ or /generic trait/.

- /Classes/ and /traits/ that take /type parameters/ are *generic*, _BUT_ the
  /types/ they generate are *parameterized*, NOT *generic*.
...
- Scala *DOESN'T* treat its /arrays/ as /covariant/.
...
- So far, all violations of type soundness involved some _reassignable field_ or
  array element.
  _HOWEVER_, _reassignable fields_ are a _SPECIAL CASE_ of the rule that
...
- =IMPORTANT=
  To verify correctness of variance annotations, the Scala compiler classifies
  all positions in a class or trait body as positive, negative, or neutral.
