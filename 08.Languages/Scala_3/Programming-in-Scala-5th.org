...
- At the *TOP* of the hierarchy is /class/ ~Any~, which defines /methods/ that
  ...
  * The ~==~ method is essentially the same as ~equals~ and ~!=~ is always the
    NEGATION of ~equals~.
    So individual /classes/ can tailor what ~==~ or ~!=~ means by _overriding the
    ~equals~ method_.

- footnote 1:
  =from Jian= All below are about the inconsistency in Java. Scala doesn't have
  those issues.
...
- The /root class/ ~Any~ has TWO /subclasses/:
  * ~AnyVal~
  * ~AnyRef~
...
- ~AnyVal~ is the /parent class/ of /value classes/ in Scala.
  ...
  * The first eight of these correspond to Java’s primitive types, and their
    values are represented at run time as Java’s primitive values. The instances
    of these classes are all written as literals in Scala. For example, 42 is an
    instance of Int, 'x' is an instance of Char, and false an instance of
    Boolean. You cannot create instances of these classes using new. This is
    enforced by the “trick” that value classes are all defined to be both
    abstract and final.

** TODO 17.2 How primitives are implemented - 382
** TODO 17.3 Bottom types - 384
** TODO 17.4 Defining your own value classes - 385
*** Avoiding a types monoculture - 386

** TODO 17.5 Intersection types - 388
** TODO 17.6 Union types - 389
** TODO 17.7 Transparent traits - 392
** TODO 17.8 Conclusion - 393
