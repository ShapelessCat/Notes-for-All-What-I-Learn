...
- In this chapter we will look at _several features *INTRODUCED* or *ENHANCED*
  in Scala 3_ that can be understood as
...
- We'll
  ...
  * look at _the /variance/ of PARAMETER and RESULT /kinds/._
    =from Jian=
    ??? can "_the /variance/ of kinds of PARAMETER and RESULT._" be clearer?
...
- Scala offers _TWO distinct ways_ to write /value-level functions/:
  ...
  * /function literals/
    #+begin_src scala
      // A method or "def"
      def incrDef(n: Int): Int = n + 1

      // A function literal
      (n: Int) => n + 1
    #+end_src

- _One DIFFERENCE_ between these two forms of /value-level function/ is
  * that
    + you CAN specify a /result type/ on a /method/,
    + BUT CAN'T on a /function literal/.

  * On the other hand,
    you can specify _the type of the entire /function literal/,_ which you
    CANNOT do with a /method/,
    BECAUSE
    a /method/ does not have a type.
    + footnote 1:
      The compiler uses a "methodic type" *internally* to facilitate /eta
      expansion/, which captures the /method's signature/, but this is not a
      type you can use in a Scala program source code.

  * You could specify the type of a /function literal/ with a /type ascription/,
    like this:
    #+begin_src scala
      ((n: Int) => n + 1): Int => Int
    #+end_src
    Since you're giving the /parameter type/ as well as the /result type/ in the
    /type ascription/, you can leave the parameter type off in the parameter
    list:
    #+begin_src scala
      (n => n + 1): Int => Int
    #+end_src

    Or, since /function literals/ do not intrinsically have a name, you can give
    this /function literal/ one by assigning it to a ~val~, then put a /type
    annotation/ on that ~val~ declaration, like this:
    #+begin_src scala
      val incrVal: Int => Int = n => n + 1
    #+end_src
