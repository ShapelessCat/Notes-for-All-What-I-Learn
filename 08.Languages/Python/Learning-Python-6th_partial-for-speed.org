...
/descriptors/, previewed in Chapter 32, provide an _alternative_ way to
*intercept* /attribute access/; they are _STRONGLY RELATED TO_ the /properties/
...
- /Descriptors/ are the _underlying implementation mechanism_ for a variety
  of _class tools_, including both:
  * /properties/
  * /slots/ =TODO=
  * play other internal roles in Python that we can safely skip here =TODO=
...
- Functionally speaking,
  ...
  * This allows us to
    _insert code_ to be run automatically on /attribute/
    + fetches
    + assignments,
...
- /Descriptors/:
  ...
  * *retain* and *use* state information of their own,
  * as well as /state/ information of the subject /instance/.

  For example, a /descriptor/ may call
  * /methods/ *available* in the /client class/,
  * as well as /descriptor-specific methods/ *it defines*.
...
- *LIKE* a /property/,
  * a /descriptor/ manages *a single, specific attribute*;
  * although
    + it can't catch all attribute accesses generically,
    + it
      - PROVIDES control over BOTH _fetch_ and _assignment_ accesses and
      - ALLOWS us to
        _change an attribute name_ freely from simple data to a computation
        _WITHOUT_ breaking existing code.

    If this sounds like /properties/, it's because it is:
    =TODO=
    as you shall see, /properties/ can be coded as /descriptors/ directly.

- *UNLIKE* /properties/,
  /descriptors/ are broader in scope, and provide a more general tool.

  * For instance, because they are coded as normal classes,
    1. /descriptors/ have their own /state/,
    2. may participate in _descriptor inheritance hierarchies_,
    3. can use _composition_ to *aggregate* objects, and
    4. provide a natural structure for coding _internal methods_ and
       _attribute documentation strings_.

**** DONE The Basics - 1227
CLOSED: [2020-10-29 Thu 00:39]
- /Descriptors/
  1. are coded as *separate* /classes/ and
  2. provide *specially named* /accessor methods/ for the /attribute access
     operations/ they wish to _INTERCEPT_ -- *get*, *set*, and *deletion*
     /methods/ in the /descriptor class/ are AUTOMATICALLY run when the
     /attribute/ _assigned to_ the /descriptor class instance/ is accessed in
     the corresponding way.

- The form of /descriptor/:
  #+begin_src python
    class Descriptor:
        "docstring goes here"
  ...
  #+end_src
  /Classes/ with _ANY_ (=from Jian= NOT ALL) of these /methods/ are CONSIDERED
  /descriptors/.

- /Descriptor's methods/ are special
  * _WHEN_
    one of their /instances/ is *assigned to ANOTHER class's* /attribute/ --
    when the /attribute/ is accessed, they are *AUTOMATICALLY invoked*.
  ...
  * *Unlike /properties/,* however, omitting a ~__set__~ allows the /descriptor
    ...
    thereby *hiding* the /descriptor/. =from Jian= In usually, I THINK, NOT expected.
...
***** DONE Read-only descriptor - 1228 - =RE-NOTE=
...
** TODO 40. Metaclasses and Inheritance - 1355
- In a sense,
  /metaclasses/ simply *extend* the _code-insertion model_ of /decorators/.
...
- On the other hand,
  /metaclasses/
  OPEN THE DOOR TO
  a variety of coding patterns that may be difficult or impossible to achieve
  otherwise, and
  * they are especially of interest to programmers (=from Jian= advanced
    frameworks or tools maker) seeking to write _flexible APIs_ or _programming
    tools_ for others to use.

- Even if you don't fall into that (=from Jian= advanced frameworks or tools
  maker) category, though, /metaclasses/ can teach you much about /Python's
  class model/ in general (as we'll see, they even impact /inheritance/), and
  are prerequisite to understanding code that employs them.
  =TODO=

- =from Jian=
  My summary to the last paragraphs:
  1. Provide you a new weapon that can be used to make better frameworks (with
     flexible API) or tools.

  2. As a side effect, through the process of learning and using /metaclasses/,
     you can understand more about /Python's class model/.
...
**** TODO Metaclasses Versus Class Decorators: Round 1 - 1361 - =NOTE=
- Having said that, it's also important to note that the /class decorators/
  described in the preceding chapter sometimes *overlap with* /metaclasses/ --
  in terms of both utility and benefit.
  * Like /metaclasses/, /class decorators/ can be used to manage both /classes/
    and their later /instances/, and their syntax makes their usage
    + similarly _explicit_ and
    + arguably _more obvious_ than helper-function calls.

*** TODO The Metaclass Model - =NOTE=
...
**** DONE Classes Are Instances of type
CLOSED: [2024-10-31 Thu 15:34]
- *PREREQUISITES*:
  "The Python Object Model", which in turn assumes knowledge of the /MRO (method
  resolution order)/ covered in _Chapter 31_ (You may want to review it).
  1. Some of its conclusions are crucial to understanding metaclasses. Namely:
     + /Instances/ are created from /classes/.
     + /Classes/ are /instances/ of a /metaclass/.
     + The ~type~ built-in is the *topmost* /metaclass/.
     + /Metaclasses/ customize ~type~ _WITH_ normal ~class~ statements.

  2. Summary:
     In short,
     /classes/ are /types/,
     /types/ are /classes/, and
     /metaclasses/ customize /types/.

     Per _Chapter 32_, the relationship between /metaclasses/ and /classes/ is
     subtly different from that between /classes/ and their /nonclass
     instances/. The latter do not generate more /instances/, and while
     attribute inheritance uses the same core mechanisms and MRO everywhere,
     /classes/ search a /metaclass tree/ that /nonclass instances/ *DO NOT*.

- It's easy to see this model's fundamentals in code:
  #+begin_src python
...
**** DONE Metaclasses Are Subclasses of Type
CLOSED: [2024-10-31 Thu 16:02]
- Q :: Why would we care that /classes/ are /instances/ of a ~type~ /class/?
- A :: It turns out that this is the _hook_ that _allows us to code
  /metaclasses/._

- Specifically, we can _CREATE_ /classes/ from /subclasses/ of ~type~ that
  customize it
  WITH NORMAL _object-oriented techniques_ and _class syntax_.

  And these /type subclasses/ are known as /metaclasses/.

- =IMPORTANT=
  _Control the way /classes/ are *created* and *augment* their behavior:_
  Specify that a /user-defined class/ be created from a /user-defined metaclass/
  instead of the /normal and default ~type~ class/.

- =IMPORTANT=
  /Metaclasses/ define a _SEPARATE, SECONDARY_ /inheritance tree/ available
  * *only to* /classes/ and
  * used as a *fallback* WHEN the normal /superclass/ search FAILS.

**** TODO Class Statement Call a type - =NOTE=
*Subclassing* the ~type~ /class/ to CUSTOMIZE it is really
ONLY _half of the /metaclass/ backstory._

ANOTHER HALF:
We still need to somehow *ROUTE* a /class's creation/ to the /metaclass/ instead
of the default ~type~.

- To comprehend the way that this is arranged, we also need to know
  * Q :: HOW ~class~ statements do their business.

**** TODO Class Statement Can Choose a type - =NOTE=
**** TODO Metaclass Method Protocol - =NOTE=

*** TODO Coding Metaclasses - 1370 - =NOTE=
...
*** TODO Inheritance: The Finale - =START HERE=
