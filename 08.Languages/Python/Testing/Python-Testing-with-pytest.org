...
** DONE 11. tox and Continuous Integration - 151
CLOSED: [2025-04-29 Tue 23:24]
- Most tools used for CI run on a server (/GitHub Actions/ is one example).
  ...
  * =from Jian=
    One value of *tox* is that we can use it instead of CI specific
    multiple-Python-version test.
    For example, it is convenient to use GitHub Actions ~stragegy~ to implement
    this, but how about if we want change CI system in the future? Use *tox*,
    and then this is no longer a problem.
...
*** DONE Introducing tox - 152
CLOSED: [2025-04-29 Tue 23:24]
- tox :: is a command-line tool that allows you to run your complete suite of
  tests in *MULTIPLE environments.*
  * _tox_ is a great _starting point_ when learning about CI.
    Although it strictly is *NOT* a CI system,

- However, _tox_ is *NOT limited to just _Python versions_.*

- In gross generalities, here's a _mental model_ for how _tox_ works:

  tox uses project information in either =setup.py= or =pyproject.toml= for the
  package under test to create an installable distribution of your package.

  It looks in =tox.ini= for a list of environments, and then for each environment,
  _tox_
  ...
  After all environments are tested,
  _tox_ reports A SUMMARY OF HOW THEY ALL DID.

*** DONE Setting Up tox - 153
CLOSED: [2025-04-29 Tue 22:30]
- Put this =tox.ini= in Project top-level
  #+NAME: ch11/cards_proj/tox.ini
  #+begin_src text
    [tox]
    envlist = py310
    isolated_build = True
  ...
  #+end_src
  * =IMPORTANT=
    + For all =pyproject.toml=-configured packages, we need to set ~isolated_build = True~.
    + For =setup.py=-configured projects using the /setuptools/ library, this line
      *can be left out.*

*** DONE Running tox - 154
CLOSED: [2025-04-29 Tue 22:30]
*** DONE Testing Multiple Python Versions - 155
CLOSED: [2025-04-29 Tue 22:32]
#+begin_src text
...
Manually pass the configuration file for _tox_:
#+begin_src shell
  tox -c tox_multiple_pythons.ini
#+end_src

*** DONE Running tox Environments in Parallel - 156
CLOSED: [2025-04-29 Tue 22:33]
Add a -p flag:
#+begin_src shell
  tox -c tox_multiple_pythons.ini -p
#+end_src

*** DONE Adding a Coverage Report to tox - 156
CLOSED: [2025-04-29 Tue 23:04]
_tox_ can add /coverage reports/ to its test runs.
...
- Use can select a specific python version with a command line flag:
  #+begin_src shell
    tox -c tox_coverage.ini -e py310
  #+end_src

*** DONE Specifying a Minimum Coverage Level - 157
CLOSED: [2025-04-29 Tue 23:05]
#+begin_src text
  [testenv]
  deps =
    pytest
...
*** DONE Passing pytest Parameters Through tox - 158
CLOSED: [2025-04-29 Tue 23:12]
#+begin_src text
  [testenv]
  deps =
    pytest
...
Then you can pass parameters to _pytest_ by adding -- and then the parameters
_pytest_ needs:
#+begin_src shell
  tox -c tox_posargs.ini -e py310 -- -k test_version --no-cov
#+end_src

*** DONE Running tox with GitHub Actions - 159
CLOSED: [2025-04-29 Tue 23:21]
#+begin_src yaml
  name: CI
    ...
    builds:
      ...
      steps:
        ...
        - name: Run Tox
          run: tox -e py
#+end_src
The ~-e py~ is a bit surprising because we don't have a ~py~ environment
specified. However, this works to select the correct version of Python
specified in our =tox.ini=.

=???=

*** DONE Review - 162
CLOSED: [2025-04-29 Tue 23:22]
*** DONE Exercises - 162
CLOSED: [2025-04-29 Tue 23:22]
*** DONE What's Next - 163
CLOSED: [2025-04-29 Tue 23:22]

** TODO 12. Testing Scripts and Applications - 165 - =START HERE=
