...
- *CONCLUSION*:
  ...
  * If you already have some experience in C and in manipulating bytes and bits,
    ...
    most of this section.* -- Thinking about concrete representations of values
    on your computer will inhibit you more than it helps you.
...
- Example:
  the value and concept of 0 should and will always have the same
  effects on all C platforms:
  * adding that value to another value x will again be x, and
  * evaluating a value 0 in a control expression will always trigger the false
    branch of the control statement.

- So far, most of our examples of values have been some kind of numbers. This
  is not an accident; it relates to one of the major concepts of C.
...
- If we abstract from the last point,
...
**** 5.1.2. Types - 46
- types :: additional properties that C associates with /values/.
...
How a concrete executable achieves this goal is left to the _discretion_ of the
compiler creators.
...
- As we have already mentioned,
  ...
  this allowed *SLACKNESS* BETWEEN _program description_ and _abstract state
  machine_ is a very valuable feature, commonly referred to as /optimization/.


  * Combined with the relative simplicity of its language description, this is
...
*** TODO 5.2. Basic types - 48 - =RE-READ=
- C has
  1. a series of /basic types/ and
  2. means of constructing /derived types/ from them
     =NEXT Section=
...
- There are *TWO levels of specification*:
  ...
  * On top of that is a _second level of specification_ comes through /header
    ...
    This second level is ORGANIZED *by type semantics,* specifying what
    properties a particular type brings to the programmer.

- Starts with the first-level specification of such types.

  From takeaway 5.1.1 #1: all basic values in C are numbers, but there are
...
- Table 5.1
  ...
  =IMPORTANT=
  =IMPORTANT=
  * Types with a gray background don't allow for arithmetic;
    they are promoted before doing arithmetic.

  * Type ~char~ is special since it can be _unsigned_ or _signed_, depending on
    the platform.
...
- Nowadays, on any realistic platform, this promotion will be a ~signed int~
  of the same value as the /narrow type/, _REGARDLESS_ of whether the /narrow
  type/ was signed.
...
- Takeaway 5.2 #2 :: Each of the four classes of base types has 3 distinct
  ...
  * Observe that among the /narrow integer types/, we have two prominent members:
    ...
    As we said earlier, for C, even these are just some sort of numbers. The 12
    remaining, unpromoted types split nicely into the four classes.
    =from Jian=
    =???=
    Why 12, how to count them to 12????????
...
- The C standard *DOESN'T prescribe* the precision of these 12 types:
  ...
  * One of the things the standard *does prescribe* is that the possible ranges
    of values for the /signed types/ and /unsigned types/ must include each
    other according to their /rank/ (in Table 5.1).

    BUT this inclusion does not need to be strict -- equal precision is OK.
...
- The comparison of the ranges of signed and unsigned types is more difficult.
  ...
  type. For the non-negative values, we have the following inclusion of the
  values of types with corresponding rank:

  That is, for a given rank, the non-negative values of the signed type fit into
  the unsigned type. On any modern platform you encounter, this inclusion is
  strict: the unsigned type has values that do not fit into the signed type. For
  example, a common pair of maximal values is 231 −1 = 2,147,483,647 for signed
  int and 232 −1 = 4,294,967,295 for unsigned int.
  =TODO: note=

- Because the interrelationship between integer types depends on the platform,
  choosing the “best” type for a given purpose in a portable way can be a
  tedious task. Luckily, we can get some help from the compiler implementation,
  which provides us with typedefs such as size_t that represent certain features.
  =TODO: note=
...
- The C standard defines a lot of other types, among them other arithmetic types
  that model special use cases. Table 5.2 list some of them.
...
- Table 5.2. Some semantic arithmetic types for specialized use cases
  ...
  * The second pair represents the type in which the preprocessor does any of its
    ...
    _BUT_
      this constraint has been relaxed; under certain circumstances, there may be
      /extended integer types/ that are wider (see _section 5.7.6_).
      =NEXT=
