...
* DONE 9. Concurrent coordination - 116
  ...
  - SO FAR
    we've _composed_ MULTIPLE /effects/
    so they run /concurrently/ and /in parallel/.
    * We noted that,
      + at first, /effects/ only allow us to
        - talk about the _values_ they produce, and
        - create NEW /effects/ by composing the outputs of other /effects/.

      + We then added the notion of a ~Fiber~ to _REPRESENT_ an /ALREADY-EXECUTING
        effect/, and with it we can start to _control_ /concurrent effects/
        by *joining* or *canceling* them.

    * Q :: BUT we haven't yet discussed *coordination* between /concurrent effects/.
           How to do it???

  - Coordination :: the behavior of one /effect/ should *depend on* another.
    * For example,
...
* TODO 10. Case study: job scheduler - 136
  ...
  - We'll first talk about jobs, how they are represented and how they changed state. Then we’ll manage those jobs so that more than one can
  run concurrently.
