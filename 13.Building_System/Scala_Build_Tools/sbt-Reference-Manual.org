...
** TODO k. Scope delegation (~.value~ lookup)
   - Summary so far:
   ...
   - Scope Delegation :: a well-defined fallback search path for sbt.
...
*** TODO Rule 4: The subproject axis search path
    #+begin_src scala
      ThisBuild / organization := "com.example"
...
**** TODO Scope axis precedennce, again
     - Exercise C: Given the following build definition:
     - Exercise D: Given the following build definition:

*** TODO Inspect command lists the delegates
*** TODO ~.value~ lookup vs dynamic dispatch
